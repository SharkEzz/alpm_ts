...
class Parser {
  ...
  private applyOptionDirective(key: string, value: string | undefined): void {
    if (value === undefined) {
      ...
      if (flagField) {
        (this.options as unknown as Record<string, boolean>)[flagField] = true;
      }
      return;
    }
    switch (key) {
  ...
  processFile(filePath: string, visited: Set<string>, depth: number): void {
    if (depth > MAX_INCLUDE_DEPTH) {
      throw new Error(`pacman.conf: Include depth exceeded ${MAX_INCLUDE_DEPTH} at ${filePath} (possible cycle)`);
    }
