...
const CATEGORY_RULES: Array<[RegExp, string]> = [
  ...
  [
    /^alpm_(checkdeps|checkconflicts|find_satisfier|find_dbs_satisfier|find_group_pkgs)$/,
    'transaction-adjacent dependency resolution',
  ],
  ...
  [
    /^alpm_(compute|decode)_(md5sum|sha256sum|signature)$|extract_keyid|checkmd5sum/,
...
function main(): void {
  ...
  if (baseline) {
    ...
    console.log(
      'Baseline: none yet - every function below reports as new. Run with --update-baseline to set one.',
    );
  }
